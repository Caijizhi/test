...
def adduser():
    ...
    try:
        ...
        atm.creatAccount(new_name,new_pwd,country,province,street,door,new_money)
    except Exception:
        print('您输入的信息不合法')
        main()
...
class Bank():
    ...
    def creatAccount(self,name,pwd,country,province,street,door,money):
        '''
        开户函数
        :return:
        '''
        ...
        addr = Address(country, province, street, door)
        user = User(account1,name,pwd,money,addr)
        ...
        if len(self.alluser) >= 100:
            ...
            try:
                sql = "insert into userinfo(account,username,password,country,province,street,door,money,bankname) values('%s','%s','%d','%s','%s','%s','%s','%f','%s')" % \
                      (account1, name, pwd, country, province, street, door, money, bank_addr)
                update(sql)
            except Exception:
                print('出现了未知错误，重新开户')
            else:
